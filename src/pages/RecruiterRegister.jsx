...
const TEAM_SIZES = [
  { label: '1-5 personnes', value: 'tiny' },
  { label: '5-15 personnes', value: 'small' },
  { label: '15-50 personnes', value: 'medium' },
  { label: '50-200 personnes', value: 'large' },
  { label: '+200 personnes', value: 'xlarge' },
];
...
export default function RecruiterRegister() {
  ...
  const handleSSO = (provider) => {
    ...
    setTimeout(() => {
      setLoading(false);
      navigate('dashboard-recruiter', { role: 'recruiter', name: 'John Recruiter', provider });
    }, 1200);
  };
  ...
  const submit = (e) => {
    ...
    setTimeout(() => {
      setLoading(false);
      navigate('dashboard-recruiter', { role: 'recruiter', name: form.name });
    }, 1400);
  };

  const isFormValid = form.name && form.email && form.password && form.confirmPassword && form.company && agreed;
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] grid-bg flex items-center justify-center px-5 py-16 relative overflow-hidden">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] bg-red-700/8 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="relative w-full max-w-[480px]">
        ...
        <div className="glass gradient-border rounded-3xl overflow-hidden">
          ...
          <div className="p-8 bg-gradient-to-b from-blue-700/5 to-transparent border-b border-white/[0.06]">
            ...
            <h1 className="text-2xl font-black text-white tracking-tight mb-1">Trouvez les meilleurs talents 🔍</h1>
            <p className="text-slate-400 text-sm">Créez votre compte et commencez à sourcer en quelques clics.</p>
          </div>

          <div className="p-8 space-y-5">
            {/* Value Proposition */}
            <div className="grid grid-cols-2 gap-2 pb-2">
              <div className="p-2.5 rounded-lg bg-white/[0.02] border border-blue-600/20">
                <div className="text-lg mb-0.5">⚡</div>
                <div className="text-xs text-slate-400 font-medium">Sourcing rapide</div>
              </div>
              <div className="p-2.5 rounded-lg bg-white/[0.02] border border-blue-600/20">
                <div className="text-lg mb-0.5">🎯</div>
                <div className="text-xs text-slate-400 font-medium">Profils qualifiés</div>
              </div>
            </div>
            ...
            <form onSubmit={submit} className="space-y-3.5">
              ...
              {/* Email */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Email professionnel</label>
              ...
              {/* Company */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Nom de l'entreprise</label>
              ...
              {/* Team Size */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Taille de l'équipe tech</label>
              ...
              {/* LinkedIn */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Profil LinkedIn (optionnel)</label>
              ...
              <button type="submit" disabled={!isFormValid || loading}
                className={`w-full py-3.5 rounded-2xl text-sm font-semibold text-white cursor-pointer flex items-center justify-center gap-2 mt-6 bg-gradient-to-r from-blue-800 to-blue-600 ${!isFormValid || loading ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-lg hover:shadow-blue-600/20'}`}>
                {loading
                  ...
                  : <><Icons.Spark className="w-4 h-4 relative z-10" /><span className="relative z-10">Activer mon compte</span></>
                }
              </button>
            </form>
          </div>
