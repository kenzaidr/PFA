...
const ROLES = [
  { id: 'student',   label: 'Étudiant / Alumni',  sub: 'Je cherche un poste tech',            icon: '🎓', grad: 'from-blue-700 to-blue-600', accent: 'cyan', dest: 'student-register' },
  { id: 'recruiter', label: 'Recruteur',           sub: 'Je cherche des talents qualifiés',    icon: '🏢', grad: 'from-blue-700 to-blue-600',       accent: 'cyan', dest: 'recruiter-register' },
  { id: 'school',    label: 'École / Pôle',        sub: 'Suivi des cohortes',                  icon: '🏫', grad: 'from-emerald-500 to-teal-600',    accent: 'emerald', dest: 'school-register' },
];
      ...
      <div className="relative w-full max-w-[480px]">
        ...
        <div className="glass gradient-border rounded-3xl overflow-hidden">
          ...
          <div className="p-8 bg-gradient-to-b from-white/[0.05] to-transparent border-b border-white/[0.06]">
            ...
            <h1 className="text-2xl font-black text-white tracking-tight mb-1">Rejoins notre communauté 🚀</h1>
            <p className="text-slate-400 text-sm">Choisis ton profil pour créer un compte adapté à tes besoins.</p>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="space-y-3 mb-6">
              {ROLES.map((r) => (
                <button key={r.id} type="button" onClick={() => setRole(r.id)}
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 transition-all ${role === r.id ? `border-${r.accent}-400 bg-${r.accent}-500/40` : 'border-white/20'}`} />
                </button>
              ))}
            </div>
            ...
            <button onClick={handleContinue}
              ...
              <span className="relative z-10">Continuer vers l'inscription</span>
            </button>
