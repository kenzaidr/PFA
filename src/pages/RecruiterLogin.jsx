...
export default function RecruiterLogin() {
  ...
  const submit = (e) => {
    ...
    setTimeout(() => {
      setLoading(false);
      navigate('dashboard-recruiter', { role: 'recruiter', name: form.email.split('@')[0] });
    }, 1400);
  };
      ...
      <div className="relative w-full max-w-[440px]">
        ...
        <div className="glass gradient-border rounded-3xl overflow-hidden">
          ...
          <div className="p-8 bg-gradient-to-b from-blue-700/5 to-transparent border-b border-white/[0.06]">
            ...
            <p className="text-slate-400 text-sm">Connecte-toi pour accéder à ton espace recruteur et trouver les meilleurs talents tech.</p>
          </div>

          <div className="p-8">
            {/* Key Benefits */}
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="p-3 rounded-xl bg-white/[0.02] border border-blue-600/20">
                <div className="text-lg mb-1">🔍</div>
                <div className="text-xs text-slate-400 font-medium">Talent Search</div>
              </div>
              <div className="p-3 rounded-xl bg-white/[0.02] border border-blue-600/20">
                <div className="text-lg mb-1">⚡</div>
                <div className="text-xs text-slate-400 font-medium">Instant Matching</div>
              </div>
            </div>
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                ...
                <input type="email" value={form.email} onChange={set('email')} required
                  placeholder="prenom@company.com"
              ...
              <button type="submit" disabled={loading}
                className={`w-full py-3.5 rounded-2xl text-sm font-semibold text-white cursor-pointer flex items-center justify-center gap-2 mt-6 bg-gradient-to-r from-blue-800 to-blue-600 hover:shadow-lg hover:shadow-blue-600/20 transition-all ${loading ? 'opacity-70' : ''}`}>
                {loading
